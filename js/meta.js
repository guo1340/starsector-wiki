(function () {
  ...
  function seoFor(route) {
    ...
    let title = S.shortName + ' - Persean Sector Command Database';
    let description = S.defaultDescription;
    let keywords = ['Starsector wiki', 'Starsector guide'];
    let ogType = 'website';
    if (info.kind === 'category') {
      title = info.category.title + ' | ' + S.titleSuffix;
      description = clip(info.category.summary + ' Tactical Starsector reference with summaries, strategy notes and related guides.');
      keywords = ['Starsector ' + info.category.title.toLowerCase(), 'Starsector wiki', 'Starsector guide'];
    } else if (info.kind === 'detail') {
      title = info.page.title + ' | ' + S.titleSuffix;
      description = clip(info.page.summary);
      keywords = ['Starsector ' + info.page.title.toLowerCase(), 'Starsector ' + info.category.title.toLowerCase(), 'Starsector wiki'];
      ogType = 'article';
  ...
  function jsonLdFor(route) {
    ...
    if (info.kind === 'home') {
      return { '@context': 'https://schema.org', '@type': 'WebSite', name: S.name, url: S.baseUrl, description: S.defaultDescription, publisher, potentialAction: { '@type': 'SearchAction', target: S.baseUrl + '/?q={search_term_string}', 'query-input': 'required name=search_term_string' } };
    }
    if (info.kind === 'detail') {
      return { '@context': 'https://schema.org', '@type': 'Article', headline: info.page.title, description: seo.description, image: seo.ogImage, url: seo.canonical, datePublished: '2026-05-24', dateModified: S.lastUpdated, author: { '@type': 'Organization', name: S.name }, publisher, mainEntityOfPage: { '@type': 'WebPage', '@id': seo.canonical } };
    }
